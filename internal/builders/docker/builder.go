...
// BuildArtifact builds the artifacts based on the input DockerBuildConfig, 
// and returns the names and SHA256 digests of the generated artifacts. 
...
// repository at the given commit hash. Returns an error if the working is a
// Git repository at a different commit.
func verifyCommit(commitHash string) (bool, error) {
	...
	if lastCommitID != commitHash {
		return false, fmt.Errorf("the repo is already checked out at a different commit")
	} else {
		return true, nil
	}
}
