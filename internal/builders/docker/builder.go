...
func CreateBuildDefinition(config *DockerBuildConfig) *BuildDefinition {
	...
	artifacts[SourceKey] = getSourceArtifact(config)
	artifacts[BuilderImageKey] = getBuilderImage(config)
...
func getSourceArtifact(config *DockerBuildConfig) ArtifactReference {
...
func getBuilderImage(config *DockerBuildConfig) ArtifactReference {
	return ArtifactReference {
...
func SetUpBuildState(config *DockerBuildConfig) (*DockerBuild, error) {
	...
	bc, err := LoadBuildConfigFromFile(config.BuildConfigPath)
	if err != nil {
